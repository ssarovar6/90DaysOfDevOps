...
groups:
  - name: system-alerts
    rules:
      ...
      - alert: ContainerDown
        expr: absent(container_last_seen{name="online-shop-app"})
        for: 1m
        labels:
          severity: critical
        annotations:
...
2. **Create an alert rule in Grafana:**
   ...
   - Query: `container_memory_usage_bytes{name="online-shop-app"} / 1024 / 1024`
...
ans = Prometheus alerts are rule-based, native, and highly scalable, evaluating PromQL queries directly on the server to handle infrastructure metrics. Grafana alerts are UI-driven, user-friendly, and versatile, allowing unified alerting across multiple data sources (Prometheus, logs, databases) in one place.

When to use each:
Use Prometheus Alerts: For low-level, high-volume infrastructure metrics requiring high reliability (e.g., Kubernetes node down, CPU usage). Use for complex, scalable, and automated alerting workflows using YAML configurations.

Use Grafana Alerts: For high-level application alerts, business logic, or when managing alerts from non-Prometheus sources (e.g., Elasticsearch, CloudWatch). Ideal for teams favoring UI-driven, dashboard-specific alerts (e.g., "Alert if this dashboard panel hits X").

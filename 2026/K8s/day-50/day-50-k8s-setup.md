...
ans: Kubernetes was created to manage multiple containers as a microservice architecture and not burdening a single server.K8s solves the problem of autoscaling and auto-healing.

2. Who created Kubernetes and what was it inspired by?
ans: Kubernetes was created by Google and inspired by Borg

3. What does the name "Kubernetes" mean?
ans:the name kubernetes comes from a Greek word helmsman and Pilot
...
ans: when command kubectl apply -f pod.yaml is executed containers are created

- What happens if the API server goes down?
ans: if API server goes down then request will not reach to controller manager and scheduler from worker node as this is the front desk to communicate to cluster. No new deployments willl happen and users will not be able to retrieve info about cluster state

- What happens if a worker node goes down?
ans: if a worker node goes down there wont be much effect as another node will take up the node 
...
I chose KIND because it is lightweight and CNCF certified
...
ans: Kubeconfig is a configuration YAML file that contains cluster information and user credentials and context also by default kubeconfig file is at `~/.kube/config
